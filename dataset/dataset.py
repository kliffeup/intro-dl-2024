...
def extract_cars_from_bboxes(data_path: str, bboxes_path: str) -> dict[str, np.ndarray]:
    ...
    for dirname, _, filenames in os.walk(data_path):
        for filename in tqdm.tqdm(filenames):
            filename2image[filename] = cv2.cvtColor(
                cv2.imread(os.path.join(dirname, filename)), cv2.COLOR_BGR2RGB
            )
...
def calc_norm_constants(
    images: Iterable[np.ndarray], augmentation_pipeline, num_augs_per_image: int = 25
...
class CarDataset(Dataset):
    def __init__(
        self,
        df: pd.DataFrame,
        filename2image: dict[str, np.ndarray],
        ...
        if is_train:
            ...
            self.classes_to_samples = {i: [] for i in range(num_classes)}
            for idx, (_, row) in enumerate(self.df.iterrows()):
                self.classes_to_samples[row.label].append(idx)
